...
aubio notes "./input/LoFiTake5Bass.wav" -s -40 > ./output/pitchedInstrument1NotesDirty.txt
...
aubio notes "./input/LoFiTake5Lead.wav" -s -40 > ./output/pitchedInstrument2NotesDirty.txt
# Notes file includes some notes without timestamps oddly
sed -e 's/[^[:space:][:cntrl:]]/&/15' -e t -e d < ./output/pitchedInstrument2NotesDirty.txt > ./output/pitchedInstrument2Notes.txt

aubio notes "./input/LoFiTake5PowerChordStabs.wav" -s -40 > ./output/pitchedInstrument3NotesDirty.txt
# Notes file includes some notes without timestamps oddly
